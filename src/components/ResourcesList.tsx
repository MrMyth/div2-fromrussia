...
const ResourcesList = () => {
  ...
  // Enhanced renderIcon function to properly handle both icon types
  const renderIcon = (Icon: React.ComponentType<any>) => {
    if (!Icon) return null;
    
    // Check if it's a Lucide icon or a React-icon
    const isFaIcon = Icon.name && Icon.name.startsWith('Fa');
